            ...
            <Tippy disabled={isFloating} content={Config.City[gs.city].festivalDesc()}>
               ...
               >
                  ...
                  <div style={{ width: "5rem" }}>
                     <FormatNumber
                        value={Math.floor(
                           (Tick.current.specialBuildings.get("Headquarter")?.building.resources.Festival ??
                              0) / FESTIVAL_CONVERSION_RATE,
                        )}
                     />
                  </div>
               </div>
                     ...
                     <FormatNumber
                        value={mapCount(tick.notProducingReasons, (v) => {
                           if (
                              ...
                              v === NotProducingReason.StorageFull
                           ) {
                              return false;
                           }
                           if (
                              options.resourceBarExcludeTurnedOffOrNoActiveTransport &&
                              (v === NotProducingReason.TurnedOff ||
                                 v === NotProducingReason.NoActiveTransports)
                           ) {
                              return false;
                           }
                           return true;
